...
%      h0 = exprnd(1/2,1,n);
     ...
     SNR  = signalToNoiseRatioModeA(P_max_D);
     N = numberOfMembers(error)
% build vectors of EE and SE for simulation and analytical cases
    for i = 1 : m  
        ...
        average_eeA_apr_N(5,i) = energyEfficiencyApproximateModeA(P, N)/10^6;
        average_seA_apr_N(5,i) = spectralEfficiencyApproximateModeA(P,N);
     end
% ------------------------ Energy efficiency subplot-------------------------
    ...
    for k = 1:4
     ...
     plot(P, average_eeA_apr_N(k,:),'LineStyle','-.','color','yellow');
    ...
    line([P_star P_star], [M-50 M + 50],'Color', 'blue','LineStyle','--');
    hold off;
    legend('Direct-D2D : Simulation','Direct-D2D : Analytical N=1','Direct-D2D : Analytical N=3','Direct-D2D : Analytical N=5','Direct-D2D : Analytical N=7','Extension');
...
% ------------------------ Spectral efficiency subplot------------------------%
     ...
     for k = 1:4
      ...
      plot(P, average_seA_apr_N(k,:),'LineStyle','-.','color','yellow');
     hold off;
     legend('Direct-D2D : Simulation','Direct-D2D : Analytical N=1','Direct-D2D : Analytical N=3','Direct-D2D : Analytical N=5','Direct-D2D : Analytical N=7','Extension');
     xlabel('UE Transmittion Power (W)');
...
% ---------------------------FUNCTIONS----------------------------------------    
   ...
   %function to find number of member in the partial sum
   function nn = numberOfMembers(error)
       N = 1;
       t = equation_solver(N);
       SNR  = signalToNoiseRatioModeA(P_max_D);
       TN = power(-1,n) * t^n / (n * factorial(n));
       while (abs(TN) > error)
           t = equation_solver(N);
           N = N * 2 + 1;
           TN = power(-1,n) * t^n / (n * factorial(n)); 
           %log(equation_solver(N)) - T_N(1/SNR,N)) + log(1/SNR);
       end
       nn = N;
   end
